...
// json
function json() {
    return src([config.paths.json+'/**/*.json'])
        .pipe(config.isDev ? sourcemaps.init() : gutil.noop())
...
// scripts
function scripts() {
    return src([config.paths.script+'/**/*.js', '!'+config.paths.script+'/lib/*.js'])
        .pipe(config.isDev ? sourcemaps.init() : gutil.noop())
...
function copycss() {
    return src( [config.paths.html+'/lib/*.css'] )
        .pipe(dest( config.pathsDev.css+'/lib' ));
};
...
function copyjs() {
    return src( [config.paths.html+'/lib/*.js'] )
        .pipe(dest( config.pathsDev.script+'/lib'));
};
